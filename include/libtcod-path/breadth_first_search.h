...
static inline void TCODPATH_bfs_set_edge(
    void* bfs_data_, const int* __restrict root_index, const int* __restrict leaf_index, int) {
  TCODPATH_BreadthFirstSearch* __restrict bfs_data = (TCODPATH_BreadthFirstSearch*)bfs_data_;
  const int distance_at_root = TCODPATH_map_get(bfs_data->distance, root_index);
  const int distance_at_leaf = TCODPATH_map_get(bfs_data->distance, leaf_index);
  const int total_distance = distance_at_root + 1;
